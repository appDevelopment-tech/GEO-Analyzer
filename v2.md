...
*Structured Report Engineering for AI Recommendation Readiness*
...
const VALIDATION_RULES = [
  ...
  { name: "has_hesitations", check: (score) => score.top_ai_hesitations.length > 0 },
  { name: "score_range", check: (score) => score.overall_score >= 0 && score.overall_score <= 100 },
  { name: "evidence_cited", check: (hesitations) => hesitations.every(h => h.evidence.length > 0) },
  { name: "fix_plan_complete", check: (score) => score.week1_fix_plan.length === 5 },
];
```
